...
def _tool_get_key_people(params: dict) -> str:
    ...
    for r in rows.data:
        ...
        linkedin = f"\n  LinkedIn: {r['linkedin_url']}" if r.get("linkedin_url") else ""
        parts.append(
            f"**{r['name']}** — {r.get('title', '?')} at {r.get('company', '?')}{linkedin}\n"
            f"  Topics: {topics}\n"
            f"  Relevance: {r.get('relevance', '')[:300]}\n"
            f"  Recent: {r.get('recent_activity', 'No recent activity')[:300]}"
        )
